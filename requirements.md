...
## 4. Main Application Layout & Views (`App.tsx`)

### 4.1. AppBar (`components/AppBar.tsx`)
...
Managed by `viewMode` state in `App.tsx` (`'main'`, `'completed'`, `'archived'`).
...
*   **Main View (`viewMode === 'main'`)**:
    ...
    *   **Tab Content**:
        *   Displays `components/ActiveTaskList.tsx` if "Active Tasks" tab is selected.
        *   Displays `components/NoteList.tsx` if "Notes" tab is selected.
    *   **Navigation Buttons**:
...
*   **Completed Tasks View (`viewMode === 'completed'`)**:
    *   Renders `components/CompletedTaskList.tsx`.
...
*   **Archived Notes View (`viewMode === 'archived'`)**:
    *   Renders `components/ArchivedNoteList.tsx`.
...
*   Contains an **"Export All Data" button**:
    *   **Opens an "Export Options" modal (`components/ExportOptionsModal.tsx`).**
...
## 5. Entry Item (`components/EntryItem.tsx`)
...
### 6.1. Detail Modal (`components/DetailModal.tsx`)
...
### 6.2. Confirm Delete Modal (`components/ConfirmDeleteModal.tsx`)
...
### 6.3. Completion Notes Modal (`components/CompletionNotesModal.tsx`)
...
### 6.4. Confirm Archive Modal (`components/ConfirmArchiveModal.tsx`)
...
### 6.5. Export Options Modal (`components/ExportOptionsModal.tsx`) - **New Component**
...
## 7. State Management (`App.tsx` & `hooks/useLocalStorage.ts`)
...
## 8. Utility Functions (`utils/dateUtils.ts`)
...
*   **Core Files**:
    *   `index.html`: Entry point, Tailwind, CSS vars for **exclusive dark theme**.
    ...
    *   `components/`: Directory for all React components.
        ...
        *   `Tabs.tsx`
        *   `EntryItem.tsx`
        *   `ActiveTaskList.tsx`, `NoteList.tsx`, `CompletedTaskList.tsx`, `ArchivedNoteList.tsx`
        *   `DetailModal.tsx`
